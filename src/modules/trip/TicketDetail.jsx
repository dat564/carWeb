...
const TicketDetail = ({ tickets, trip, breakPointSortasc }) => {
    ...
    React.useEffect(() => {
        if (!breakPointSortasc) return;
        setTotalPrices(
            seatSelected.reduce((total) => {
                return total + Number(breakPointSortasc?.[0]?.price);
            }, 0)
        );
    }, [breakPointSortasc, seatSelected, sortTickets]);

    const handleChangeBreakPoint = (breakPoint) => {
        setTotalPrices(
            seatSelected.reduce((total) => {
                return total + Number(breakPoint.price);
            }, 0)
        );
    };
    ...
    const handleContinue = () => {
        ...
        //         break_point: pickpoinData.break_point,
        //     }));
        ...
        <Spin spinning={loading}>
            <div className='w-full mt-5 font-medium transition-all'>
                ...
                <p className='font-normal text-red-500'>
                    Lưu ý: Sử dụng app đặt vé để có thể sử dụng voucher
                </p>
                <div className='flex justify-between w-full px-5 py-5 mt-5 border-t border-slate-400 footer'>
                    {current === 1 ? (
                        ...
                        <div>Tổng số ghế: 11</div>
                    )}
                    <div className='flex items-center gap-5'>
                        <span>Tổng cộng: 120.000đ</span>
