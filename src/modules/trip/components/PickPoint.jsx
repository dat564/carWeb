...
const PickPoint = ({ trip, handleChangeBreakPoint, isView = false }, ref) => {
            ...
            return (
                <li className='flex items-baseline gap-4 text-lg font-medium'>
                    ...
                    <div>
                        <span>
                            {formatTime(
                                convertDatetimeAndFormat(trip?.departure_time)
                            )}
                        </span>{" "}
                        • {trip?.route_start}
    ...
    const renderEndPoint = () => {
        return trip.break_point?.map((item) => (
            <li
                className='flex items-baseline gap-4 text-lg font-medium'
                key={item.id}
            >
                <div>
                    {!isView && (
                            ...
                            onChange={(e) => {
                                if (e.target.checked) {
                                    ...
                                    const value = form.getFieldValue(
                                        `endPoint_value_${item.id}`
                                    );
                                    handleChangeBreakPoint(item);

                                    resultRef.current = {
                                        ...resultRef.current,
                                        end_point: value || item.end_point,
                                    };
                                }
                            }}
                        />
                ...
                <div>
                    <span>
                        {formatTime(
                            convertDatetimeAndFormat(item?.scheduled_end_time)
                        )}
                    </span>{" "}
                    • {item?.name}
                    <div className='text-base font-normal'>
    ...
    useImperativeHandle(
        ...
        () => ({
            getData: () => {
                return {
                    ...resultRef?.current,
                    break_point_id: selectedBreakPoint?.id,
                    break_point: selectedBreakPoint,
                };
            },
        }),
        [selectedBreakPoint]
    ...
    return (
        <ProForm
            form={form}
            className='flex justify-center gap-10 mt-5 mb-5 text-lg'
            submitter={false}
        >
