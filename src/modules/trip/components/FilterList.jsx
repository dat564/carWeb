...
const FilterList = () => {
  ...
  return (
    <div className="w-full p-5 text-base bg-white rounded-2xl">
      <h2 className="mb-4 text-xl font-semibold">Lọc</h2>
      <ProForm submitter={false}>
        <SliderWithLabel label="Giờ đi" min={0} max={24} defaultValue={[0, 24]} marks={{ 0: '0h', 24: '24h' }} />
        <SliderWithLabel
          label="Giá vé"
          min={0}
          max={2000000}
        ...
        />
        <SliderWithLabel label="Đánh giá" min={0} max={5} defaultValue={[0, 5]} marks={{ 0: '0⭐', 5: '5⭐' }} />
        <Button
...
const SliderWithLabel = ({ label, min, max, defaultValue, ...props }) => {
  return (
    <>
      ...
      <Slider min={min} max={max} range defaultValue={defaultValue} {...props} />
    </>
  );
};
